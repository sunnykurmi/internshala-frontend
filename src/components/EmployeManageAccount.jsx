...
export default function EmployeManageAccount() {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    try {
      await dispatch(employeupdate(formData, user._id));
      dispatch(asynccurrentemploye());
    } catch (error) {
      //(error.response.data);
    }
  ...
  const handleSubmit2 = async (e) => {
    ...
    if (formData.password.length >= 6) {
      ...
      try {
        await dispatch(ChangePassword4(formData, user._id));
        dispatch(asynccurrentemploye());
      } catch (error) {
        //(error.response.data);
      }
  ...
  const handleSubmit3 = async (e) => {
    ...
    if (confirmation.toLowerCase() === "yes") {
      try {
        await dispatch(Delete2(user._id));
        dispatch(asynccurrentemploye());
      } catch (error) {
        //(error.response.data);
      }
  ...
  return (
    <>
      ...
      <div className="w-full flex gap-[5vh] flex-col items-center justify-center py-[10vh] px-[20vh] h-[80vh] ">
        <div className="flex  max-[600px]:flex-col  w-[45vw] justify-between  items-center">
